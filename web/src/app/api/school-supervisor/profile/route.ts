...
export const GET = requireSchoolSupervisor(async (request: NextRequest, session) => {
  try {
    ...
    const profile = await supervisorService.getSchoolSupervisorProfile(supervisorId);
...
export const PATCH = requireSchoolSupervisor(async (request: NextRequest, session) => {
  try {
    const supervisorId = session.user.userReferenceId;
    ...
    const updated = await supervisorService.updateSchoolSupervisorProfile(supervisorId, body);
