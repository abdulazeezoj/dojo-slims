...
export function useStudentWeekData(
  ...
  return useQuery({
    queryKey: ["industry-supervisor-week", studentId, weekId],
    queryFn: async () => {
      const response = await apiClient.get<ApiResponse<WeekData>>(
        `/api/industry-supervisor/students/${studentId}/weeks/${weekId}`,
      );
  ...
  return useMutation({
    mutationFn: async (data: AddCommentData) => {
      await apiClient.post(
        `/api/industry-supervisor/students/${studentId}/weeks/${weekId}/comment`,
        data,
      );
    },
    onSuccess: () => {
